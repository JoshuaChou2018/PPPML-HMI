...
for name in ['Scanner-C_B32.npz', 'Scanner-C_B33.npz', 'Scanner-C_B34.npz', 'Scanner-C_B35.npz']:
    print("process", name, ',', total - processed, 'left')
    if os.path.exists(image_save_dict + name[:-4] + '.png'):
        print("processed")
        processed += 1
        continue
    ...
    """
    Here we save the masks for lungs, airways and blood vessels, and save the enhanced array.
    """
    ...
    """
    Here we give freq illustration for the enhanced array
    """
    temp_dict = '/home/zhoul0a/Desktop/prognosis_project/visualize/open_access/enhance_effect/'
    enhance_slice = np.clip(enhanced_array[:, :, mid_z], -0.05, 0.15)
    Functions.image_save(enhanced_array[:, :, mid_z], temp_dict + name[:-4], high_resolution=True, gray=True)
