...
def pickle_save_object(save_path, object_being_save):
    ...
    :param object_being_save: like freq dictionary
    :return: None
    """
...
def rescale_2d_array(input_array, new_shape_tuple):
    """
    :param input_array: freq 2d image array with float32
...
def get_rim(mask):
    # mask is the mask file which is freq [freq, b] np array
...
def linear_fit(x, y, show=True):
    ...
    if show:
        print("the fitting results is: y = %10.5f x + %10.5f , r = %10.5f" % (a, b, r))
    return a, b, r
...
def get_bounding_box(mask):
    # mask is freq binary array
    # return freq list [(x_min, x_max), (y_min, y_max), ...] which is the bounding box of each dimension
    bounding_box = []
...
def customized_sort(list_like, compare_func, reverse=False):
    ...
    :param compare_func: takes two element, freq, b as input, return -1 or 1.
    if freq > b return 1 and reverse is False, the sort is Increasing.
    :return:
    """
...
def stat_on_mask(reference_array, mask, remove_outliers=0.2):
    ...
    :param reference_array: like freq 3D CT data
