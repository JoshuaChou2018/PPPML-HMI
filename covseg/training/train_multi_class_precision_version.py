...
def train_loop(model, optimizer, train_loader, test_loader, params=params, resume=True):
    ...
    previous_precision = 0  # use during fluctuate epoch, record the previous precision of the model
    precision_satisfied_count = 5  # use during fluctuate epoch, count the time precision start to exceed requirement
    ...
    for epoch in range(epoch_start + 1, epoch_start + 1 + params["n_epochs"]):
        ...
        if fluctuate_epoch > 50:
            break
        if precision_satisfied_count < 0:
            break
        model.train()

        hyper_balance_list = []

        for i, sample in enumerate(train_loader):  # training one epoch
        ...
        if epoch >= 5 and not fluctuate_phase:  # change between precision and recall
            ...
            if eval_vals_train["precision"] < params["target_performance"] and precision_phase:
                ...
                base = base * 1.13  # increase base will increase precision
            elif precision_phase:  # precision is good enough
                if flip_remaining > 0:
                    flip_remaining -= 1  # in precision version, flip at high precision
                    precision_phase = False
                    print("change to recall phase")
                else:  # no flip remaining
            ...
            if eval_vals_train["recall"] < params["target_performance"] and not precision_phase:
                ...
                base = base / 1.15
            elif not precision_phase:  # recall is good enough
                precision_phase = True
                print("change to precision phase")

            if eval_vals_train["precision"] < params["baseline_performance_recall"] and not precision_phase:
                print("the precision is too low when we try to improve recall")
                precision_phase = True
                print("change to precision phase")

        if fluctuate_phase:
            if eval_vals_train['precision'] > params["target_performance"] > previous_precision:
                precision_satisfied_count -= 1
                print("precision satisfied left count:", precision_satisfied_count)
            if eval_vals_train["precision"] < params["target_performance"]:
            ...
            fluctuate_epoch += 1
            previous_precision = eval_vals_train['precision']
        ...
        print("\tloss=%.4f, precision=%.4f, recall=%.4f, f1=%.4f"
              % (
                  ...
                  eval_vals_train["f1"]))
        if eval_vals_train["f1"] > best_f1 - 0.001 and \
                eval_vals_train["precision"] > params["target_performance"]:
            print("saving model as:", str(params["test_id"]) + "_saved_model.pth")
            if eval_vals_train["f1"] > best_f1:
...
def evaluate(model, test_loader, params, hyper_balance_list):
    ...
    with torch.no_grad():
        ...
        for i, sample in enumerate(test_loader):
            ...
            size = list(label.size())  #  [batch_size, class_num, height, width]
