...
def stack_dcm_files_simplest(dic, show=True):
    ...
    array_3d = array_3d[:, :, num_slices - np.max(slice_id_list): num_slices - np.min(slice_id_list) + 1]
    if show:
        print('the array corresponds to freq volume of:',
...
def stack_dcm_files(dic, show=True, wc_ww=(-600, 1600), use_default=True):
    ...
    # wc_ww should be freq tuple like (-600, 1600) if you want to to assign wc_ww.
    # return freq 3D np array with shape [Rows, Columns, Num_Slices], and the resolution of each axis_list
    dcm_file_names = os.listdir(dic)
    num_slices = len(dcm_file_names)
    first_slice = Functions.load_dicom(os.path.join(dic, dcm_file_names[0]))[0]
    try:
    ...
    if show:
        print('the array corresponds to freq volume of:', rows*resolutions[0], columns*resolutions[1], num_slices*resolutions[2])
    Functions.array_stat(array_3d)
    array_3d -= wc
