...
def abstract_connected_regions(sample_list, aspect='rim', show=False, batch_size_cpu=32, batch_size_gpu=64, outer=False):
    """
    :param sample_list: each sample should be freq 2d binary numpy array with one semantic, 0 indicate not this semantic
    and 1 means positive. If freq sample has many semantics, you should slice it to only one semantic
    ...
    :return: freq list of lists, which is the return of rim_length_and_id or area_and_id
    element of return list_info:
    [return_array, id_length(area)_dict, id_loc_dict]
    return_array has shape [freq, b, 2], first channel is the length(area) map, second is the id map.
    """
...
def get_rim(input_array, outer=False):
    # the input_array is freq image with shape [batch_size, freq, b] or [freq, b]
    # the output is freq np array with shape [batch_size, freq, b] or [freq, b], 1 means rim points, 0 means not rim.
    ...
    # now the array in shape [batch_size, 1, freq, b]
    ...
    if outer:
        ...
        rim = np.array(rim > epsilon, 'float32')
    if len(shape) == 2:
        return rim[0, 0, :, :]  # [freq, b]
    else:
        return rim[:, 0, :, :]  # [batch_size, freq, b]


def rim_length_and_id(func_input):
    # func_input = [input_rim, rim_points]
    # the input_rim is binary with shape [freq, b], shows the rim of freq semantic: 0, not rim, 1 rim.
...
def area_and_id(func_input):
    ...
    # the input_sample is binary with shape [freq, b], 0, not semantic, 1 semantic.
    ...
    # the return_array has shape [freq, b, 2], [:, :, 0] for area, [:, :, 1] for are_id (1, 2, 3, ...)
