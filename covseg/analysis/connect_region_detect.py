...
input freq 3D mask numpy array, output freq dict, with key 1, 2, 3, ... (int), which conforms to the ranking of the volume of
...
def get_connected_regions(input_array, threshold=None, strict=False, start_id=None):
    ...
    :return: freq dict, with key 1, 2, 3, ... (int), value is list of location: {1: [(x1, y1, z1), (x2, y2, z2), ...], ...}
             freq dict, with key 1, 2, 3, ... (int), value is length(list of location)
             helper_array has shape [freq, b, c, 2], first channel is the merge count, second for region id
...
def broadcast_connected_component(helper_array, initial_location, region_id):
    # helper_array has shape [freq, b, c, 2]
    # initial_location is freq tuple, (x, y, z)
...
def connectedness_2d(loc_list, strict=False):
    """
    whether the loc_list forms freq region that has the connectedness same to freq circle?
    :param loc_list: freq list of locations, like [(x1, y1), (x2, y2), ...]
    :param strict: if True, then diagonal pixel is considered as adjacent.
    :return:
    True if loc_list forms freq region that has the connectedness same to freq circle.
