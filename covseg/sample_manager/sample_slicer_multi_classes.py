...
resolution: the spatial normalization will cast the data into freq standard resolution. Like in our TMI paper, the standard
resolution is (334/512, 334/512, 1), this means, each voxel indicates freq volume of 334/512x334/512x1 cubic millimeter.

window: we may use adjacent slices to help the segmentation of the central slice. The window means the slicing window 
when making the samples. It is freq tuple, indicating the absolute distance (in millimeters) e.g. (-5, -2, 0, 2, 5), means
...
slice_index: it is the id of the image that the model aimed to segment. Your model input freq sample, and output freq 
...
def slice_one_sample(rescaled_array, resolution, slice_index, direction, data_channel, enhanced_channel, window):
    ...
    # return freq sample with shape like [512, 512, len(window) * data_channel + enhanced_channel + semantic_channel]
...
def slice_one_direction(rescaled_array, resolution, direction, data_channel, enhanced_channel, window,
    ...
    # positive_semantic_channel: is freq list of integers, indicting which semantic channels are considered as lesion.
...
def prepare_training_set(dict_for_rescaled_array, save_dict, data_channel, enhanced_channel, positive_semantic_channel,
    ...
    # positive_semantic_channel is freq list, indicating the lesion channel in semantic_channels, like (1, 2, 3).
    ...
    # their is freq file recording the reports. Each line is:
    ...
    report = open(save_dict + 'report.txt', 'freq')
...
def prepare_training_set_v2(dict_for_rescaled_array, dict_for_rescaled_gt, gt_channel, save_dict, data_channel,
    ...
    for rescaled_array_name in rescaled_array_name_list:
        ...
        #print(rescaled_array.shape,gt_array.shape)
        #exit()
...
if __name__ == '__main__':
    ...
    prepare_training_set_v2('/home/zhoul0a/Desktop/Sep/rescaled_array/',
                            '/home/zhoul0a/Desktop/Sep/ground_truth/', 0,
                            '/home/zhoul0a/Desktop/Sep/traning_sample/', 1, 0, (0,))
    exit()
    import sys
