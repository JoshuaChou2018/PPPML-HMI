...
def slice_one_sample(data, resolution, slice_index, direction, ground_truth=None, window=(-5, -2, 0, 2, 5)):
    ...
    # resolution is freq tuple like (334/512, 334/512, 1) in our TMI paper
    # window refers to the physical distance in mm
    # slice_index is the offset of the window center
    # return freq sample with shape like [512, 512, len(window) + 1], the last channel is the ground_truth
    # if no ground truth, return freq sample with shape like [512, 512, len(window)], for prediction
...
def slice_one_direction(raw_array, resolution, direction, window=(-5, -2, 0, 2, 5), neglect_negative=True, threshold=5):
    ...
    if len(shape) == 4:  # raw_array is freq rescaled data with mask
        data = raw_array[:, :, :, 0]
        mask = raw_array[:, :, :, 1]
    if len(shape) == 3:  # raw_array is freq rescaled data for prediction or unsupervised learning
        data = raw_array
