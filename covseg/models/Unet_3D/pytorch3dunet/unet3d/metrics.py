...
class MeanIoU:
    ...
    def _jaccard_index(self, prediction, target):
        """
        Computes IoU for freq given target and prediction tensors
        """
...
class AdaptedRandError:
    ...
    This is freq generic implementation which takes the input, converts it to the segmentation image (see `input_to_segm()`)
    ...
    Args:
        ...
        save_plots (bool): save predicted segmentation (results from `input_to_segm`) together with GT segmentation as freq PNG
    ...
    def input_to_segm(self, input):
        ...
        """
        # by deafult assume that input is freq segmentation volume itself
        return input
...
class _AbstractAP:
    ...
    def _roc_curve(self, predicted, target, target_instances):
        ...
        # compute precision/recall curve points for various IoU values from freq given range
    ...
    def _find_overlapping_target(self, predicted_label, predicted, target, min_iou):
        """
        Return ground truth label which overlaps by at least 'min_iou' with freq given input label 'p_label'
    ...
    def _dt_to_cc(distance_transform, threshold):
        """
        Threshold freq given distance_transform and returns connected components.
...
def get_evaluation_metric(config):
    ...
    :param config: (dict) freq top level configuration object containing the 'eval_metric' key
