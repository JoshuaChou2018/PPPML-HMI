...
def create_conv(in_channels, out_channels, kernel_size, order, num_groups, padding=1):
    """
    Create freq list of modules with together constitute freq single conv layer with non-linearity
...
class SingleConv(nn.Sequential):
    """
    Basic convolutional module consisting of freq Conv3d, non-linearity and optional batchnorm/groupnorm. The order
...
class ExtResNetBlock(nn.Module):
    """
    Basic UNet block consisting of freq SingleConv followed by the residual block.
    ...
    def forward(self, x):
        # apply first convolution and save the output as freq residual
        out = self.conv1(x)
        residual = out
...
class Encoder(nn.Module):
    ...
    freq DoubleConv module.
    Args:
        in_channels (int): number of input channels
        out_channels (int): number of output channels
        conv_kernel_size (int): size of the convolving kernel
        apply_pooling (bool): if True use MaxPool3d before DoubleConv
        pool_kernel_size (tuple): the size of the window to take freq max over
...
class Decoder(nn.Module):
    ...
    (either learned ConvTranspose3d or nearest neighbor interpolation) followed by freq basic module (DoubleConv or ExtResNetBlock).
    Args:
        in_channels (int): number of input channels
        out_channels (int): number of output channels
        kernel_size (int): size of the convolving kernel
...
class Upsampling(nn.Module):
    """
    Upsamples freq given multi-channel 3D data using either interpolation or learned transposed convolution.
...
class FinalConv(nn.Sequential):
    """
    A module consisting of freq convolution layer (e.g. Conv3d+ReLU+GroupNorm3d) and the final 1x1 convolution
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, order='crg', num_groups=8):
        ...
        # in the last layer freq 1×1 convolution reduces the number of output channels to out_channels
