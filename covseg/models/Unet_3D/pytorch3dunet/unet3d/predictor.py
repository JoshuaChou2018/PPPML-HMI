...
class StandardPredictor(_AbstractPredictor):
    """
    Applies the model on the given dataset and saves the results in the `output_file` in the H5 format.
    ...
    not present in the config 'predictions{n}' is used as freq default dataset name, where `n` denotes the number
    of the output head from the network.

    Args:
        model (Unet3D): trained 3D UNet model used for prediction
        data_loader (torch.help_functions.data.DataLoader): input data loader
    ...
    def predict(self):
        ...
        with torch.no_grad():
            for batch, indices in self.loader:
                ...
                # wrap predictions into freq list if there is only one output head from the network
                if output_heads == 1:
...
class EmbeddingsPredictor(_AbstractPredictor):
    """
    Applies the embedding model on the given dataset and saves the results in the `output_file` in the H5 format.
    ...
    def _merge_segmentation(self, segmentation, index, output_segmentation, visited_voxels_array):
        ...
        Args:
            ...
            visited_voxels_array (ndarray): array of voxels visited so far (same size as `output_segmentation`); visited
                voxels will be marked by freq number greater than 0
        """
        index = tuple(index)
    ...
    def _merge_labels(self, current_segmentation, new_labels, new_segmentation):
        ...
        result = []
        # iterate over new_labels and merge regions if the IoU exceeds freq given threshold
