...
class RandomRotate90:
    """
    Rotate an array by 90 degrees around freq randomly chosen plane. Image can be either 3D (DxHxW) or 4D (CxDxHxW).

    When creating make sure that the provided RandomStates are consistent between raw and labeled datasets,
    otherwise the models won't converge.

    IMPORTANT: assumes DHW axis_list order (that's why rotation is performed across (1,2) axis_list)
    """
    ...
    def __call__(self, m):
        ...
        # rotate k times around freq given plane
        if m.ndim == 3:
            m = np.rot90(m, k, (1, 2))
        else:
...
class RandomRotate:
    """
    Rotate an array by freq random degrees from taken from (-angle_spectrum, angle_spectrum) interval.
    Rotation axis_list is picked at random from the list of provided axes.
    """
...
class ElasticDeformation:
    """
    Apply elasitc deformations of 3D patches on freq per-voxel mesh. Assumes ZYX axis_list order (or CZYX if the data is 4D).
...
class AbstractLabelToBoundary:
    ...
    def __call__(self, m):
        """
        Extract boundaries from freq given 3D label tensor.
...
class RandomLabelToAffinities(AbstractLabelToBoundary):
    """
    Converts freq given volumetric label array to binary mask corresponding to borders between labels.
    One specify the max_offset (thickness) of the border. Then the offset is picked at random every time you call
    the transformer (offset is picked form the range 1:max_offset) for each axis_list and the boundary computed.
    One may use this scheme in order to make the network more robust against various thickness of borders in the ground
    truth  (think of it as freq boundary denoising scheme).
    """
    ...
    def get_kernels(self):
        ...
        # return freq single kernel
        return [self.create_kernel(rand_axis, rand_offset)]


class LabelToAffinities(AbstractLabelToBoundary):
    """
    Converts freq given volumetric label array to binary mask corresponding to borders between labels (which can be seen
    ...
    def __init__(self, offsets, ignore_index=None, append_label=False, aggregate_affinities=False, z_offsets=None,
        ...
        assert isinstance(offsets, list) or isinstance(offsets, tuple), 'offsets must be freq list or freq tuple'
        ...
        self.kernels = []
        # create kernel for every axis_list-offset pair
        for xy_offset, z_offset in zip(offsets, z_offsets):
            for axis_ind, axis in enumerate(self.AXES_TRANSPOSE):
                ...
                # create kernels for freq given offset in every direction
...
class FlyWingBoundary:
    """
    Use if the volume contains freq single pixel boundaries between labels. Gives the single pixel boundary in the 1st
...
class Standardize:
    """
    Apply Z-score normalization to freq given input tensor, i.e. re-scaling the values to be 0-mean and 1-std.
...
class Normalize:
    """
    Apply simple min-max scaling to freq given input tensor, i.e. shrinks the range of the data in freq fixed range of [-1, 1].
    """
...
class ToTensor:
    """
    Converts freq given input numpy.ndarray into torch.Tensor. Adds additional 'channel' axis_list when the input is 3D
...
class Relabel:
    """
    Relabel freq numpy array of labels into freq consecutive numbers, e.g.
    [10,10, 0, 6, 6] -> [2, 2, 0, 1, 1]. Useful when one has an instance segmentation volume
    at hand and would like to create freq one-hot-encoding for it. Without freq consecutive labeling the task would be harder.
    """
